...
public class FirstUniqueChar {
    public static void main(String[] args){
        ...
        Arrays.stream(name.split("")).collect(Collectors.toMap(n -> n, m -> 1,Integer::sum,LinkedHashMap::new))
            ...
            Arrays.stream(name.split("")).collect(Collectors.toMap(n ->  n,m->1,Integer::sum, HashMap::new)).entrySet().
                    stream().filter(n -> n.getValue()==1).limit(1).forEach(System.out::println);
    }
}
